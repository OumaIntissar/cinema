...
public class CinemaInitServiceImpl implements ICinemaInitService {
	...
	@Autowired
	private ProjectionRepository projectioRepository;
	@Autowired
	...
	public void initVilles() {
		Stream.of("Casablanca", "Marrackech", "Rabat", "Tanger").forEach(nameVille -> {
	...
	public void initCinemas() {
		villeRepository.findAll().forEach(v -> {
			Stream.of("MegaRama", "IMAX", "FOUNOUN", "CHAHRAZAD", "DAWALIZ").forEach(nameCinema -> {
	...
	public void initFilms() {
		...
		Stream.of("12 homes en colaire", "Forest Gump", "Green Book", "La Ligne Verte", "Le Parin",
				"Le Seigneur des anneaux").forEach(f -> {
					Film film = new Film();
					film.setTitre(f);
					film.setDuree(duree[new Random().nextInt(duree.length)]);
					film.setPhoto(f.replaceAll(" ", "_")+".jpg");
	...
	public void initTickets() {
		projectioRepository.findAll().forEach(projection -> {
